...
// Uses @remix namespace like Vite's /@vite/client pattern.
export const HMR_RUNTIME_PATH = '/@remix/hmr/runtime.js'
...
function convertObjectPatternToExpression(pattern: ObjectPattern): ObjectExpression {
  let properties = pattern.properties.map((prop) => {
...
function extractComponentFromFunctionDecl(func: FunctionDeclaration): ComponentInfo | null {
  ...
  let renderBody = extractRenderBodyFromBlock(func.body)
  if (!renderBody) return null

  return {
    name: func.identifier.value,
    params: func.params.map((p) => p.pat),
    body: func.body,
    renderBody,
    span: func.span,
  }
...
function extractComponentFromVarDecl(decl: VariableDeclaration): ComponentInfo | null {
  ...
  if (init.type === 'FunctionExpression') {
    ...
    let renderBody = extractRenderBodyFromBlock(funcExpr.body)
    if (!renderBody) return null

    return {
      name,
      params: funcExpr.params.map((p) => p.pat),
      body: funcExpr.body,
      renderBody,
      span,
    }
  ...
  if (init.type === 'ArrowFunctionExpression') {
    ...
    if (arrowExpr.body.type === 'BlockStatement') {
      ...
      let renderBody = extractRenderBodyFromBlock(body)
      if (!renderBody) return null
    ...
    if (exprBody.type === 'ArrowFunctionExpression' || exprBody.type === 'FunctionExpression') {
      ...
      // Extract the render body from the inner function
      ...
      if (innerFunc.type === 'ArrowFunctionExpression') {
        ...
        let innerReturn = innerFunc.body.stmts.find(
          (s): s is ReturnStatement => s.type === 'ReturnStatement',
        )
...
// Extract render body from a block statement (finds return statement)
function extractRenderBodyFromBlock(body: BlockStatement): Expression | null {
  let returnStmt = body.stmts.find((s): s is ReturnStatement => s.type === 'ReturnStatement')
  ...
  if (returnedFunc.type === 'ArrowFunctionExpression') {
    return returnedFunc.body as Expression
  }
...
function generateHmrComponent(
  ...
  let implBody: Statement[] = [
    ...
    // __hmr_register(moduleUrl, name, param, () => renderBody)
    createExpressionStatement(
      createCallExpression('__hmr_register', [
        createStringLiteral(moduleUrl),
        createStringLiteral(name),
        paramRef,
        createArrowFunction(transformedRenderBody),
      ]),
    ),

    // return () => __hmr_call(param)
    createReturnStatement(createArrowFunction(createCallExpression('__hmr_call', [paramRef]))),
  ]
...
export async function transformComponent(
  ...
  for (let item of ast.body) {
    ...
    if (item.type === 'ExportDeclaration') {
      ...
      component = extractComponentFromVarDecl(item)
    }
