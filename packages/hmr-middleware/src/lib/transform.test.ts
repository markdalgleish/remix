...
import { transformComponent, maybeHasComponent, HMR_RUNTIME_PATH } from './transform.ts'
...
function normalizeHashes(code: string): string {
  // Match hash strings in __hmr_setup calls: __hmr_setup(handle, __s, 'h1abc2d', ...)
  return code.replace(/__hmr_setup\([^,]+,\s*[^,]+,\s*'h[a-z0-9]+'/g, (match) => {
    return match.replace(/'h[a-z0-9]+'/, "'HASH'")
  })
}
...
function Counter__impl(handle) {
  let __s = __hmr_state(handle)
  if (__hmr_setup(handle, __s, 'h47rocz', () => {
    __s.count = 0
    __s.name = 'test'
    __s.active = true
  })) {
...
function Counter__impl(handle) {
  let __s = __hmr_state(handle)
  if (__hmr_setup(handle, __s, 'huzp8kd', () => {
    __s.count = helper()
  })) {
    __hmr_request_remount(handle)
    return () => null
...
function Internal__impl(handle) {
  let __s = __hmr_state(handle)
  if (__hmr_setup(handle, __s, 'hwsvxcg', () => {
    __s.value = 42
  })) {
    __hmr_request_remount(handle)
    return () => null
